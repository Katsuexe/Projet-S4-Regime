...
<section class="section">
  <div class="card">
    <div class="card-body">
      ...
      <form action="<?= site_url('admin/regimes/update/'.$regime['id']) ?>" method="POST">
        <?= csrf_field() ?>
          ...
          <div class="col-md-8">
            <div class="row g-3">
              ...
              <div class="col-12">
                ...
                <textarea name="description" class="form-control" rows="3"><?= esc($regime['description'] ?? '') ?></textarea>
              </div>

              <div class="col-md-6">
                <label class="form-label">Delta poids min (kg) <span class="text-danger">*</span></label>
                <input type="number" name="delta_poids_min" class="form-control" step="0.1" value="<?= esc($regime['delta_poids_min']) ?>" required>
              </div>

              <div class="col-md-6">
                <label class="form-label">Delta poids max (kg) <span class="text-danger">*</span></label>
                <input type="number" name="delta_poids_max" class="form-control" step="0.1" value="<?= esc($regime['delta_poids_max']) ?>" required>
              </div>
              ...
              <div class="col-12 mt-4">
                ...
                <div class="row g-3">
                  <div class="col-md-4">
                    ...
                    <input type="number" name="pct_viande" class="form-control pct-input" min="0" max="100" value="<?= esc($regime['pct_viande']) ?>" required>
                  </div>
                  <div class="col-md-4">
                    <label class="form-label text-primary">Poisson</label>
                    <input type="number" name="pct_poisson" class="form-control pct-input" min="0" max="100" value="<?= esc($regime['pct_poisson']) ?>" required>
                  </div>
                  <div class="col-md-4">
                    <label class="form-label text-warning">Volaille</label>
                    <input type="number" name="pct_volaille" class="form-control pct-input" min="0" max="100" value="<?= esc($regime['pct_volaille']) ?>" required>
                  </div>
                </div>
          ...
          <div class="col-md-4 border-start">
            ...
            <table class="table table-sm" id="dureesTable">
              ...
              <tbody id="dureesBody">
                ...
                <?php else: ?>
                  <?php foreach ($regime['durees'] as $i => $d): ?>
                  <tr>
                    <td><input type="number" name="durees[<?= $i ?>][jours]" class="form-control form-control-sm" min="1" value="<?= esc($d['duree_jours']) ?>" required></td>
                    <td><input type="number" name="durees[<?= $i ?>][prix]" class="form-control form-control-sm prix-normal" step="0.01" min="0" value="<?= esc($d['prix']) ?>" required></td>
                    <td><input type="number" class="form-control form-control-sm prix-gold" step="0.01" value="<?= esc($d['prix'] * 0.85) ?>" readonly></td>
                    <td><button type="button" class="btn btn-sm btn-outline-danger remove-row"><i class="bi bi-x"></i></button></td>
                  </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", () => {
    let dureeIndex = <?= isset($regime['durees']) && is_array($regime['durees']) ? count($regime['durees']) : 1 ?>;
